...
- `RTLLMv1.1/`
- `RTLLM2.0/`
- `RTLLM_seven_modified/`: the seven-task sanitized testbench-analysis package used for the focused RTLLM follow-up study
...
- `dataset-root` should point to one of the bundled benchmark folders such as `data_Evalhuman`, `RTLLMv1.1`, or `RTLLM2.0`.
- `trace-root` is user-defined and should point to a local output directory where you want a run to write traces and summaries.
- The current public package does **not** include local retest-result artifacts such as machine-specific rerun summaries.
- If you want to reproduce those retests, regenerate the result files locally in your own environment rather than relying on any removed private experiment outputs.
- The release archive `156_rtllm_data.zip` is a convenience bundle of the included benchmark folders and does not need extra path edits before use.

## Where To Configure Things

You do **not** need to edit the pipeline source code just to set local paths.
For normal use, configuration happens in only two places:

1. API/provider settings:
   edit a local `.env` file created from `SpecLens_procedure/.env.example`
2. Run-specific paths:
   pass them on the command line through `--dataset-root`, `--trace-root`, and
   optionally `--env-file`

Concretely:

- Edit `SpecLens_procedure/.env` to set `OPENAI_API_KEY`, `LLM_PROVIDER`, and
  any optional model overrides.
- Set `--dataset-root` in the command you run. This should point to one of the
  bundled benchmark directories such as `data_Evalhuman`, `RTLLMv1.1`, or
  `RTLLM2.0`.
- Set `--trace-root` in the command you run. This is your own output directory
  for traces and summaries.
- Use `--env-file` only if you want to keep the key file somewhere other than
  the default local `.env`.

If you want to inspect or permanently change the CLI defaults, the default
argument values are defined in:

- `SpecLens_procedure/realbench_safe_pipeline_156/cli.py`
- `SpecLens_procedure/realbench_safe_pipeline_RTLLM/cli.py`

For most users, however, passing the desired values on the command line is the
...
- Use relative paths if you run commands from the `SpecLens_procedure/` directory.
- Use absolute paths only if you prefer them locally; no absolute machine-specific paths are required by the released code.
- The code does not assume my original workstation layout. Any old local-result references preserved in notes or retest artifacts were intentionally replaced by placeholders and are not required for normal pipeline execution.
...
For the focused seven-task package:

- `RTLLM_seven_modified/` stores the cleaned testbenches, analysis notes, and retest summaries for that subset.
- Its `retest_results/` directory is documentation-oriented; if you rerun the experiments locally, you should expect your own output paths and timestamps to differ.
