...
def write_llm_io(task_output_dir: Path, llm_io: dict) -> dict[str, str]:
    ...
    file_order = [
        ...
        ("direct_few_shots", "001bb_direct_few_shots.json"),
            ...
            bundled_local_rounds.append(
                {
                    ...
                    "main_resolution_input_stimulus_case": local_round.get(
                        "main_resolution_input_stimulus_case"
                    ),
                    "stimulus_refined_in_main_resolution": local_round.get(
...
def build_run_summary(
    ...
    missing_mismatches_summary_by_variant: dict[str, list[str]] = {}
    ...
    for trace in traces:
        ...
        if _trace_missing_final_mismatches_summary(trace):
            missing_mismatches_summary_by_variant.setdefault(variant, []).append(task_id)
    ...
    summary = {
        "missing_mismatches_summary_by_variant": {
            variant: sorted(task_ids)
            for variant, task_ids in sorted(missing_mismatches_summary_by_variant.items())
        },
        "variants": by_variant,
    }
    if run_timing:
