...
def _postprocess_cluster_candidate_output_initialization(
    *,
    ...
    The final candidate sent to testbench verification is always materialized as
    a dedicated `_output_init` candidate id, even when the postprocess decides
    that no Verilog text changes are needed. This keeps the verified artifact
    folder name stable and makes the final tested candidate obvious on disk.
    """
...
""".strip()
    ...
    revised_candidate = CandidateVerilog(
        ...
        generation_notes=(
            f"{candidate.generation_notes} Post-processed before verification through output-init review."
            if not changed
            else f"{candidate.generation_notes} Post-processed before verification to add spec-grounded "
...
def _prefilter_requirement_scenarios_with_fixed_initial_candidates(
    ...
    if screening_jobs:
        max_workers = min(len(screening_jobs), max(1, config.task_max_prefilter_workers))
...
- if the specification or target requirement is waveform-driven and sequential, read a trigger-edge row as the observable post-edge result determined by the input that was already stable immediately before that edge, and do not infer an extra cycle of input persistence unless the specification explicitly requires it
...
- For edge-detection tasks such as pedge/anyedge or tasks described as detecting 0->1, 1->0, or any change from one clock cycle to the next, if the transition is determined by comparing the value sampled on the previous positive edge with the value sampled on the current positive edge, then the correct output pulse belongs to the post-edge observation of that current sampling edge itself, and any design that first stores the detection result and only outputs it on the next clock edge has introduced an incorrect extra pipeline delay unless the specification explicitly requires that extra delay.
- For outputs that indicate whether another state element should update on the current clock edge, interpret them according to the current visible pre-edge state and the state update caused on that same edge, not as a separately delayed indication that appears only after the resulting updated outputs are already visible, unless the specification explicitly requires such a registered delay.
- Do not infer or prefer extra externally observable behavior that the specification does not state. If the specification explicitly describes some update conditions but does not specify a different behavior outside them, do not invent an additional default update, transition, pulse, or output response beyond what is stated.
- For FSMs that parse a continuous byte stream into fixed-length messages, do not treat the cycle that asserts the message-complete pulse as a dead or separator cycle; that completion cycle still belongs to the stream, and the very next input byte must already be judged as the next potential stream byte unless the specification explicitly inserts an extra idle or gap cycle.
...
- For Moore-style FSM specifications that label states with outputs such as A (0), B (1), or similar notation, treat the parenthesized value as the output of the current state itself. Prefer the trajectory whose observed output matches the post-edge current state reached on that edge, and do not prefer a look-ahead interpretation that mixes the old state with the current input unless the specification explicitly defines a Mealy output.
- For waveform-reconstruction tasks, judge correctness from the provided sampled observations themselves. If a candidate's behavior only matches after inventing an unobserved higher-level simplification, that is weaker evidence than directly matching the sampled precondition-to-postcondition mapping shown by the waveform or state/output table.
- For waveform-reconstruction tasks, if an output changes throughout a phase where the clock remains at one fixed logic level rather than only immediately after edges, treat that as evidence for level-sensitive behavior during that phase rather than for an edge-triggered flip-flop.
- For history-shift or rollback specifications that say a history value before a branch is concatenated with the branch outcome, preserve the stated bit ordering exactly. If the specification says the youngest branch is stored in bit 0 / the least-significant bit, prefer the trajectory consistent with appending the new branch result at bit 0 and shifting older history toward more-significant bits.
- For timers or counters that must hold each visible remaining-count value for exactly K cycles, prefer the trajectory that keeps each displayed value for exactly K sampled clock cycles, without an extra boundary cycle before decrementing or before entering the done state.
""".strip()
    record = backend.complete_json_record(system_prompt=system_prompt, user_prompt=user_prompt)
    parsed = record["parsed_response"]
...
- if the specification or target requirement is waveform-driven and sequential, write the constraint using the interpretation that a trigger-edge row shows the observable post-edge result determined by the input that was already stable immediately before that edge, and do not encode an extra cycle of input persistence unless the specification explicitly requires it
...
- For edge-detection tasks such as pedge/anyedge or tasks described as detecting 0->1, 1->0, or any change from one clock cycle to the next, if the transition is determined by comparing the value sampled on the previous positive edge with the value sampled on the current positive edge, then the correct output pulse belongs to the post-edge observation of that current sampling edge itself, and any design that first stores the detection result and only outputs it on the next clock edge has introduced an incorrect extra pipeline delay unless the specification explicitly requires that extra delay.
- For outputs that indicate whether another state element should update on the current clock edge, write the constraint in terms of the current visible pre-edge state and the state update caused on that same edge, not as a separately delayed indication that appears only after the resulting updated outputs are already visible, unless the specification explicitly requires such a registered delay.
- Do not write a constraint that adds extra externally observable behavior not stated by the specification. If the specification explicitly describes some update conditions but does not specify a different behavior outside them, do not introduce an additional default update, transition, pulse, or output response beyond what is stated.
- For FSMs that parse a continuous byte stream into fixed-length messages, do not write a constraint that treats the message-complete pulse cycle as a dead or separator cycle; unless the specification explicitly inserts a gap, the next input byte after a completed message must already be interpreted under the same ongoing stream rules as the next potential message byte.
- For framed serial protocols, if the stop bit is incorrect when first expected, do not write a constraint that treats a later return of the line to logic 1 as retroactively completing that same frame; that later high only re-establishes the idle or ready condition for the next frame unless the specification explicitly says late completion is allowed.
- For Moore-style FSM specifications that label states with outputs such as A (0), B (1), or similar notation, write the constraint in terms of the output belonging to the current state itself after the triggering clock edge. Do not encode a look-ahead rule that mixes the old state with current inputs unless the specification explicitly defines a Mealy output.
- For waveform-reconstruction tasks, write the constraint to preserve the concrete sampled mapping shown by the waveform or state/output table. Do not summarize the task into a guessed higher-level pattern if that guessed pattern is not directly supported by the observed samples.
- For waveform-reconstruction tasks, if an output changes while the clock remains at a fixed logic level, do not write a constraint that forces edge-triggered behavior; instead preserve the observed level-sensitive behavior during that clock phase when the specification/waveform supports it.
- For history-shift or rollback specifications that say a history value before a branch is concatenated with the branch outcome, write the constraint so the stated bit ordering is preserved exactly. If the specification says the youngest branch is stored in bit 0 / the least-significant bit, the new branch result must be appended at bit 0 and older history shifted toward more-significant bits.
- For timers or counters that must hold each visible remaining-count value for exactly K cycles, write the constraint so each displayed value lasts for exactly K sampled clock cycles, with no extra entry or exit cycle at count-value boundaries or at the transition into done.
- If the specification explicitly says that some output or state change occurs on the next clock cycle, the following clock cycle, or after one additional cycle, do not write a constraint that removes that real specification-mandated delay. Preserve the explicitly stated delay exactly.
...
def _run_llm_verilog_cluster_resolution(
    ...
    cluster_record = {
        ...
        "effective_stimulus_cases": list(result.get("stimulus_cases") or []),
    }
    cluster_trace = {
...
def _run_requirement_stimulus_constraint_loop(
    ...
    for screened_entry in screened_scenario_entries:
        ...
        for local_round_index in range(1, max_rounds_per_requirement + 1):
            ...
            main_resolution_input_stimulus_case = _deepcopy_jsonable(current_stimulus_case)
            ...
            effective_stimulus_cases = list(cluster_result.get("stimulus_cases") or [])
            if not effective_stimulus_cases or not isinstance(effective_stimulus_cases[0], dict):
                raise RuntimeError(
                    "main cluster resolution returned no usable effective stimulus case "
                    f"for requirement {target_requirement_id}, local round {local_round_index}"
                )
            effective_stimulus_case = _deepcopy_jsonable(effective_stimulus_cases[0])
            stimulus_refined_in_main_resolution = (
                isinstance(main_resolution_input_stimulus_case, dict)
                and _stimulus_semantic_key(main_resolution_input_stimulus_case)
                != _stimulus_semantic_key(effective_stimulus_case)
            )
            current_stimulus_case = effective_stimulus_case
            if main_resolution_input_stimulus_case is None:
                stimulus_history = [_deepcopy_jsonable(current_stimulus_case)]
            elif stimulus_refined_in_main_resolution:
                stimulus_history.append(_deepcopy_jsonable(current_stimulus_case))
            elif stimulus_history:
                stimulus_history[-1] = _deepcopy_jsonable(current_stimulus_case)
            else:
                stimulus_history = [_deepcopy_jsonable(current_stimulus_case)]
            ...
            local_round_payload: dict[str, Any] = {
                ...
                "main_resolution_input_stimulus_case": _deepcopy_jsonable(
                    main_resolution_input_stimulus_case
                ),
                "stimulus_refined_in_main_resolution": stimulus_refined_in_main_resolution,
...
def build_task_base(
    ...
    llm_io = {
        "requirements_constraint_selfplanning": {
            ...
            "cluster_feedback_mode": cluster_feedback_mode,
        ...
        "llm_verilog_cluster_disambiguation_constraints": {
            **requirement_stimulus_loop_io,
            "selected_mode": cluster_feedback_mode,
        },
