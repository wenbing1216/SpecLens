...
def generate_candidate_from_spec_with_constraints(
    ...
    system_prompt = (
        "You are a professional Verilog coding expert. "
        "Generate Verilog code from the specification. "
        "The specification is the primary source of truth. "
        "The provided design constraints are secondary hints derived from prior analysis and may be incomplete, redundant, or imperfect. "
        "Use the constraints only when they clarify behavior that is already supported by the specification. "
        "If any constraint conflicts with the specification, waveform, interface, or other constraints, follow the specification and ignore the conflicting constraint. "
        "Do not implement behavior solely because it appears in the constraints."
    )
    constraints_text = json.dumps(constraints, ensure_ascii=False, indent=2) if constraints else "[]"
    user_prompt = f"""
Generate Verilog code from the following specification.

Treat the design constraints only as auxiliary hints.
Use them to resolve ambiguity only when they are consistent with the specification.
Do not let the constraints override the specification.
