...
from realbench_safe_pipeline_156.entropy import semantic_entropy_from_counts
...
def _get_runtime_monitor() -> Any | None:
    ...
    try:
        module = import_module("realbench_safe_pipeline_156.runtime_monitor")
    except Exception:
        return None
...
- For edge-detection tasks, a transition is determined by comparing the value sampled on the previous positive edge with the value sampled on the current positive edge. The correct output pulse belongs to the post-edge observation of that current sampling edge itself; do not first store the detection result and then output it one clock later unless the specification explicitly requires that extra delay.
- For directional bump behaviors, interpret bump_left and bump_right as front-side obstacle sensors relative to the current walking direction: while walking left, bump_left may reverse direction but bump_right alone must not; while walking right, bump_right may reverse direction but bump_left alone must not, unless the specification explicitly says rear-side bumps also change direction.
- For synchronous active-low reset behaviors, if resetn is sampled as 1 on a given positive clock edge, the FSM must follow its normal non-reset transition on that same edge rather than waiting an extra clock cycle; do not require reset deassertion to be stable for an additional cycle before leaving the reset state unless the specification explicitly says so.
- For counters with enable outputs that indicate when higher digits should increment, treat each enable as an externally visible combinational indication derived from the current visible count state, not as a separately registered pulse that appears one cycle later after the lower digit has already rolled over, unless the specification explicitly requires that extra output register delay.
- If a specification says a condition may continue with no upper bound and a later event depends on whether it has lasted for more than N clock cycles, do not use a free-running bounded counter whose current numeric value alone determines the decision at the later event. Count the entry edge as cycle 1, and once the duration has exceeded the threshold, preserve that exceeded-threshold fact monotonically until the later event is evaluated, for example by using a saturating counter or a sticky flag, rather than allowing wraparound or truncation to make a long run appear safe again.
- For FSMs that parse a continuous byte stream into fixed-length messages, treat the cycle that asserts the message-complete pulse as still belonging to the stream itself rather than as a dead or separator cycle; after a complete message is recognized, the very next input byte must immediately be interpreted as the next potential stream byte under the same boundary rule.
- For framed serial protocols, if the stop bit is incorrect when first expected, do not treat a later return of the line to logic 1 as retroactively completing that same frame; that later high only re-establishes the idle or ready condition for the next frame unless the specification explicitly says late completion is allowed.
- For tasks that explicitly ask you to derive one-hot next-state and output equations by inspection from a provided state table and encoding, implement only the stated combinational equations over the given one-hot state bits and inputs, and do not add any default recovery, invalid-state handling, or other extra behavior for unspecified encodings unless the specification explicitly requires it.
- When selecting a slice from a packed vector using a selector-derived bit index, ensure the arithmetic that computes the starting bit position is wide enough for the full vector range; do not use a shift or multiply on a narrow selector if that would truncate higher address bits and mis-select upper slices.
- For Moore-style FSM specifications that label states with outputs such as A (0), B (1), or similar notation, treat the parenthesized value as the output of the current state itself. In those tasks, the observable output must be derived from the post-edge current state reached on that clock, not from a speculative look-ahead expression that mixes the old state with current inputs unless the specification explicitly defines a Mealy output.
- For waveform-reconstruction tasks, infer behavior directly from the provided sampled observations rather than inventing a higher-level pattern first. If the waveform exposes a state/output table or a sampled transition table, first reconstruct the concrete sampled mapping from pre-edge conditions to post-edge outputs/state and then implement that mapping faithfully.
- For waveform-reconstruction tasks, if an output changes repeatedly throughout the interval where the clock remains at one fixed level rather than only immediately after clock edges, treat that as evidence for level-sensitive behavior during that clock phase rather than forcing an edge-triggered flip-flop interpretation.
- For waveform-reconstruction tasks that provide only sampled observations at clock edges, do not invent an extra one-cycle history register or delayed staging unless the listed samples force that interpretation. Prefer the smallest state/output update rule that matches every shown edge-to-edge observation exactly. If the waveform shows an output changing during a sustained clock level rather than only immediately after edges, treat that as evidence of level-sensitive behavior during that phase instead of forcing an edge-triggered interpretation.
- For FSM specifications that explicitly say reset release causes a one-cycle pulse after the next clock edge, or that a detected multi-cycle input sequence causes an output on the following clock cycle, preserve those stages exactly. Do not move such an output earlier to the same edge as reset release or sequence completion. If the specification says an output remains asserted while another input is monitored for up to two following cycles, keep it asserted throughout that full observation window and only change it afterward according to the stated rule.
- For exact-cycle timer, counter, and clock tasks, preserve boundary counts literally. If a visible count value is supposed to last K cycles, it must last exactly K sampled clock cycles, including the final visible zero interval when specified. For 12-hour clocks, toggle the AM/PM indicator only on the enabled increment from 11:59:59 to 12:00:00. For reset-triggered fixed-length enable pulses, emit exactly the specified number of consecutive asserted post-edge cycles for one reset event, and do not repeatedly retrigger the pulse while reset remains continuously high.
- For history-shift or rollback specifications that say a history value before a branch is concatenated with the branch outcome, preserve the stated bit ordering exactly. If the specification says the youngest branch is stored in bit 0 / the least-significant bit, then shifting in a new result means appending that result at bit 0 and shifting older history toward more-significant bits; do not drop the wrong end of the history vector.
- For branch-history rollback tasks, preserve the stated bit ordering exactly. If the specification says bit 0 / the least-significant bit holds the youngest branch, then a normal prediction update must shift older history toward more-significant bits and insert the new prediction result at bit 0, and a rollback that loads train_history concatenated with train_taken must produce {{train_history[30:0], train_taken}} in q[31:0] notation rather than dropping the wrong end of the history vector.
- For tasks whose next state is defined by a precise transition table, precedence rule, or per-bit update rule, compute the full next state only from the pre-edge state and the stated rule, without reusing partially updated bits, reversing neighbor indexing, or inventing extra priority rules. In particular, when a specification says falling overrides directional bumps and preserves the pre-fall direction, obey that precedence exactly; when a specification gives a Galois LFSR tap pattern or a cellular-automaton neighbor table, implement that exact table-driven next-state function from the old state in one step.
- For timers or counters that must hold each visible remaining-count value for exactly K cycles, implement the counting window so each displayed value lasts for exactly K sampled clock cycles, without adding an extra entry or exit cycle at the boundary between adjacent count values or between counting and done.
- When the specification explicitly says that some output or state change occurs on the next clock cycle, the following clock cycle, or after one additional cycle, preserve that stated delay exactly. Do not remove a real specification-mandated one-cycle delay just because many other tasks are harmed by accidental extra latency.
...
- When converting a sequential scenario into a stimulus case, preserve the timing semantics of the specification and scenario exactly: do not rewrite a post-edge observable result determined by the pre-edge stable input as an extra one-cycle-later effect, and do not mention a next-cycle delay unless the specification explicitly requires it.
...
- If a scenario depends on whether a condition has lasted for more than N cycles and the specification says there is no upper bound on that duration, do not stop at a just-above-threshold example; when needed for coverage, use a substantially longer run than N so the stimulus can distinguish correct threshold-preserving behavior from finite-width counter wraparound.
"""
...
- If a sequential scenario depends on whether a condition has lasted for more than N cycles and the specification says there is no upper bound on that duration, do not stop at a just-above-threshold example when strengthening coverage; when needed, extend the run to be substantially longer than N so the refined stimulus can distinguish correct threshold-preserving behavior from finite-width counter wraparound.
- Use only ports from the majority interface.
...
- When refining a sequential stimulus case, preserve the timing semantics of the specification and scenario exactly: do not rewrite a post-edge observable result determined by the pre-edge stable input as an extra one-cycle-later effect, and do not mention a next-cycle delay unless the specification explicitly requires it.
...
def remove_comments(verilog_code: str) -> str:
    """Strip Verilog comments so downstream regex parsing sees only structural code. Many later parsing steps rely on regexes and string scans rather than a full Verilog parser."""
    code_without_single_comments = re.sub(r"//.*", "", verilog_code)
    return re.sub(r"/\*[\s\S]*?\*/", "", code_without_single_comments)


def _strip_leading_fence_language_token(code: str) -> str:
    """Drop a stray standalone language token from fenced-code contents."""
    ...
    if not lines:
        return stripped
    if lines[0].strip().lower() not in {"verilog", "systemverilog", "sv"}:
        return stripped
    remainder = "\n".join(lines[1:]).strip()
    if re.search(r"\bmodule\b", remainder, re.IGNORECASE):
        return remainder
    return stripped
...
def extract_verilog_code(text: str) -> str:
    ...
    if fenced_blocks:
        return "\n\n".join(
            _strip_leading_fence_language_token(block)
            for block in fenced_blocks
            if _strip_leading_fence_language_token(block)
        )
...
class VerilogStimulusClusterPipeline:
    def __init__(
        ...
        *,
        ...
        self.max_candidate_workers = max(1, int(max_candidate_workers))
    ...
    def build_resolution_output(
        ...
        if semantic_entropy <= RESOLUTION_ZERO_ENTROPY_TOLERANCE:
