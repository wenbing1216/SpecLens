"""Command-line entrypoint for the Evalhuman-safe pipeline."""
...
from .pipeline import SUPPORTED_PIPELINE_VARIANTS, default_config, discover_tasks, run_dataset_pipeline
from .trace import build_run_summary, write_run_summary


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Evalhuman-safe pipeline")
    parser.add_argument("--dataset-root", default="data_Evalhuman", help="Dataset root to scan")
    parser.add_argument("--trace-root", default="pipeline_runs_156", help="Directory for traces")
    parser.add_argument(
        "--task",
        action="append",
        default=None,
    ...
    parser.add_argument("--max-workers", type=int, default=2, help="Number of tasks to run in parallel")
    parser.add_argument(
        "--task-max-candidate-workers",
        type=int,
        default=4,
        help="Number of candidate-generation workers inside one task",
    )
    parser.add_argument(
        "--task-max-prefilter-workers",
        type=int,
        default=4,
        help="Number of prefilter scenario-screening workers inside one task",
    )
    ...
    parser.add_argument(
        "--model",
        default=None,
        help="Optional OpenAI model override for unified CLI runs",
    )
    parser.add_argument(
        "--reasoning-effort",
        default=None,
        choices=["minimal", "low", "medium", "high"],
        help="Optional OpenAI reasoning effort override for unified CLI runs",
    )
